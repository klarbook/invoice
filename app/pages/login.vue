...
async function login() {
  try {
    ...
    errorMsg.value = "Invalid login data"
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="min-h-screen bg-neutral-100 px-8 pt-24">
    <div class="mx-auto max-w-sm rounded-lg border border-neutral-50 bg-white p-8 shadow">
      <h1 class="mb-4 text-center text-2xl font-semibold text-neutral-900">Invoice</h1>

      <form @submit.prevent="login" class="flex flex-col gap-4">
        <div class="flex flex-col gap-1">
          <d-label for="email">Email</d-label>
          <d-input v-model="email" type="email" id="email" name="email" required placeholder="Your e-mail address" />
        </div>
        ...
        <div class="flex flex-col gap-2">
          <DButton :loading type="submit" text-center>Log in</DButton>
          <DButton to="/forgot-password" variant="secondary" text-center>Forgot your password?</DButton>
        </div>
