...
export default defineEventHandler(async (event) => {
  ...
  if (!user.resetPasswordToken || !user.resetPasswordTokenValidUntil) throw createError({ statusCode: 401, message: "Bad credentials" })
  ...
  await useDrizzle().update(users).set({ password: hashedPassword, resetPasswordToken: null, resetPasswordTokenValidUntil: null }).where(eq(users.id, user.id))
