...
export default defineEventHandler(async (event) => {
  ...
  const invoice = await useDrizzle().transaction(async (tx) => {
    ...
    const invoiceNo = `${contact.customerNo}-${contact.invoiceSequence.toString().padStart(4, "0")}`
    ...
    await tx
      .update(contacts)
      .set({ invoiceSequence: contact.invoiceSequence + 1 })
      .where(and(eq(contacts.id, body.recipientId), eq(contacts.organisationId, secure.organisationId)))
