...
async function backgroundProcessEmail({ email }: { email: string }) {
  ...
  const resetLink = `https://invoice.klarbook.com/reset-password?token=${passwordResetToken}`
  ...
  try {
    const emailResult = await postmark.sendEmail({
      from: "Invoice by Klarbook <app@invoice.klarbook.com>",
      to: user.email,
      subject: "Reset Password",
      htmlBody:
        `<p>Hello ${user.name},</p>` +
        `<p>You have received a password reset request for your Invoice account. To reset your password, please click the following link:</p>` +
        `<p><a href="${resetLink}">Reset Password</a></p>` +
        `<p>If you didn't request this email, you can safely ignore it.</p>` +
        `<p>Thank you<br />Your Invoice Team</p>`,
    })
